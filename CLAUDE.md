...
If two skills happen to share a doc (e.g. both `higgsfield-generate` and `higgsfield-soul` have `troubleshooting.md`), keep separate copies. Drift between skills is acceptable as long as each skill is internally consistent. CI verifies self-containment on every PR.
...
- `higgsfield-soul/SKILL.md` — `version:` in frontmatter.
